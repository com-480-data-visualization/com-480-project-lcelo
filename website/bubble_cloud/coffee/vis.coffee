...
Bubbles = () ->
  ...
  updateLabels = () ->
    ...
    # ASLAM TAILLE TEXTE
    label
      .style("font-size", (d) ->  Math.max(8, (rValue(d) / 6)) + "px") # Math.max(0.1, rScale(rValue(d) / 5000000))
      .style("width", (d) -> 0.2 * rScale(rValue(d)) + "px")
...
texts = [
  {key:"Covid",file:"top_covid.csv",name:"Covid-19"}
